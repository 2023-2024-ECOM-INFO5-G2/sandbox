...
/**
 * Performance test for the MesureEPA entity.
 */
public class MesureEPAGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all mesureEPAS").get("/api/mesure-epas").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new mesureEPA")
                        .post("/api/mesure-epas")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"valeur\": 0" + ", \"date\": \"2020-01-01T00:00:00.000Z\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_mesureEPA_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created mesureEPA").get("${new_mesureEPA_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created mesureEPA").delete("${new_mesureEPA_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MesureEPA entity").exec(scn);
