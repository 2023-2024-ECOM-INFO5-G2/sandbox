...
/**
 * Performance test for the MesureAlbumine entity.
 */
public class MesureAlbumineGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all mesureAlbumines").get("/api/mesure-albumines").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new mesureAlbumine")
                        .post("/api/mesure-albumines")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"valeur\": 0" + ", \"date\": \"2020-01-01T00:00:00.000Z\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_mesureAlbumine_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created mesureAlbumine").get("${new_mesureAlbumine_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created mesureAlbumine").delete("${new_mesureAlbumine_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MesureAlbumine entity").exec(scn);
