...
/**
 * Performance test for the Infirmiere entity.
 */
public class InfirmiereGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all infirmieres").get("/api/infirmieres").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new infirmiere")
                        .post("/api/infirmieres")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody("{" + "\"idInfirmiere\": 0" + ", \"prenom\": \"SAMPLE_TEXT\"" + ", \"nom\": \"SAMPLE_TEXT\"" + "}")
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_infirmiere_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created infirmiere").get("${new_infirmiere_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created infirmiere").delete("${new_infirmiere_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Infirmiere entity").exec(scn);
