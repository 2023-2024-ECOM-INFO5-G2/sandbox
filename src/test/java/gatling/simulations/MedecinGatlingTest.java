 ...
 */
public class MedecinGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all medecins").get("/api/medecins").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new medecin")
                        .post("/api/medecins")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" + "\"idMedecin\": 0" + ", \"firstName\": \"SAMPLE_TEXT\"" + ", \"lastName\": \"SAMPLE_TEXT\"" + "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_medecin_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created medecin").get("${new_medecin_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created medecin").delete("${new_medecin_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Medecin entity").exec(scn);
