...
/**
 * Performance test for the AideSoignant entity.
 */
public class AideSoignantGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all aideSoignants").get("/api/aide-soignants").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new aideSoignant")
                        .post("/api/aide-soignants")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"idSoignant\": 0" + ", \"prenom\": \"SAMPLE_TEXT\"" + ", \"nom\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_aideSoignant_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created aideSoignant").get("${new_aideSoignant_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created aideSoignant").delete("${new_aideSoignant_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AideSoignant entity").exec(scn);
