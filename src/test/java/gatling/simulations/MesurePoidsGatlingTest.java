...
/**
 * Performance test for the MesurePoids entity.
 */
public class MesurePoidsGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all mesurePoids").get("/api/mesure-poids").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new mesurePoids")
                        .post("/api/mesure-poids")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"valeur\": 0" + ", \"date\": \"2020-01-01T00:00:00.000Z\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_mesurePoids_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created mesurePoids").get("${new_mesurePoids_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created mesurePoids").delete("${new_mesurePoids_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MesurePoids entity").exec(scn);
