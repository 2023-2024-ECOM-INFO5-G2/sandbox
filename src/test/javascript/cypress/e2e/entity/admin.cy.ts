...
describe('Admin e2e test', () => {
  ...
  const adminSample = { idAdmin: 21743, prenom: 'dans la mesure où isoler', nom: 'au-dessus de' };
  ...
  describe('new Admin page', () => {
    ...
    it('should create an instance of Admin', () => {
      cy.get(`[data-cy="idAdmin"]`).type('161');
      cy.get(`[data-cy="idAdmin"]`).should('have.value', '161');

      cy.get(`[data-cy="prenom"]`).type('spécialiste touchant bè');
      cy.get(`[data-cy="prenom"]`).should('have.value', 'spécialiste touchant bè');

      cy.get(`[data-cy="nom"]`).type('sitôt que');
      cy.get(`[data-cy="nom"]`).should('have.value', 'sitôt que');
