      ...
      if (this.mesureEPA.id) {
        this.mesureEPAService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showInfo(this.t$('g2ecomApp.mesureEPA.updated', { param: param.id }));
          })
          .catch(error => {
            this.isSaving = false;
        ...
        this.mesureEPAService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showSuccess(this.t$('g2ecomApp.mesureEPA.created', { param: param.id }).toString());
          })
          .catch(error => {
            this.isSaving = false;
            this.alertService.showHttpError(error.response);
