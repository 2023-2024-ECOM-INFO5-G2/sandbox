<template>
  <div>
    <h2 id="page-heading" data-cy="InfirmiereHeading">
      <span v-text="t$('ecom02App.infirmiere.home.title')" id="infirmiere-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('ecom02App.infirmiere.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'InfirmiereCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="t$('ecom02App.infirmiere.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && infirmieres && infirmieres.length === 0">
      <span v-text="t$('ecom02App.infirmiere.home.notFound')"></span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="infirmieres">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="t$('ecom02App.infirmiere.idInfirmiere')"></span></th>
            <th scope="row"><span v-text="t$('ecom02App.infirmiere.prenom')"></span></th>
            <th scope="row"><span v-text="t$('ecom02App.infirmiere.nom')"></span></th>
            <th scope="row"><span v-text="t$('ecom02App.infirmiere.idInfirmiere')"></span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="infirmiere in infirmieres" :key="infirmiere.id" data-cy="entityTable">
            ...
            <td>{{ infirmiere.idInfirmiere }}</td>
            <td>{{ infirmiere.prenom }}</td>
            <td>{{ infirmiere.nom }}</td>
            <td>
              <span v-for="(idInfirmiere, i) in infirmiere.idInfirmieres" :key="idInfirmiere.id"
                >{{ i > 0 ? ', ' : '' }}
                <router-link class="form-control-static" :to="{ name: 'PatientView', params: { patientId: idInfirmiere.id } }">{{
                  idInfirmiere.id
                }}</router-link>
              </span>
            </td>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span id="ecom02App.infirmiere.delete.question" data-cy="infirmiereDeleteDialogHeading" v-text="t$('entity.delete.title')"></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-infirmiere-heading" v-text="t$('ecom02App.infirmiere.delete.question', { id: removeId })"></p>
      </div>
