      ...
      <table class="table table-striped" aria-describedby="repas">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="t$('ecom02App.repas.idRepas')"></span></th>
        ...
        <tbody>
          <tr v-for="repas in repas" :key="repas.id" data-cy="entityTable">
            ...
            <td>
              <span v-for="(idRepas, i) in repas.idRepas" :key="idRepas.id"
                >{{ i > 0 ? ', ' : '' }}
                <router-link class="form-control-static" :to="{ name: 'PatientView', params: { patientId: idRepas.id } }">{{
                  idRepas.id
                }}</router-link>
              </span>
            </td>
