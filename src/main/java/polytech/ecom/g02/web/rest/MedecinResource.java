...
/**
 * REST controller for managing {@link polytech.ecom.g02.domain.Medecin}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class MedecinResource {
    ...
    public ResponseEntity<Medecin> partialUpdateMedecin(
        ...
        Optional<Medecin> result = medecinRepository
            ...
            .map(existingMedecin -> {
                if (medecin.getIdMedecin() != null) {
                    existingMedecin.setIdMedecin(medecin.getIdMedecin());
                }
                if (medecin.getFirstName() != null) {
                    existingMedecin.setFirstName(medecin.getFirstName());
                }
                if (medecin.getLastName() != null) {
                    existingMedecin.setLastName(medecin.getLastName());
                }
