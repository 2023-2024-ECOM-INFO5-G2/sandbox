...
public class RepasResource {
    ...
    public ResponseEntity<Repas> partialUpdateRepas(
        ...
        Optional<Repas> result = repasRepository
            ...
            .map(existingRepas -> {
                if (repas.getIdRepas() != null) {
                    existingRepas.setIdRepas(repas.getIdRepas());
                }
     ...
     */
    @GetMapping("/repas")
    public List<Repas> getAllRepas(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get all Repas");
        if (eagerload) {
            return repasRepository.findAllWithEagerRelationships();
        } else {
    ...
    public ResponseEntity<Repas> getRepas(@PathVariable Long id) {
        ...
        Optional<Repas> repas = repasRepository.findOneWithEagerRelationships(id);
