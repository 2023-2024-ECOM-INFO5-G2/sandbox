...
/**
 * REST controller for managing {@link polytech.ecom.g02.domain.Infirmiere}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class InfirmiereResource {
     ...
     */
    @PostMapping("/infirmieres")
    public ResponseEntity<Infirmiere> createInfirmiere(@Valid @RequestBody Infirmiere infirmiere) throws URISyntaxException {
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/infirmieres/{id}")
    public ResponseEntity<Infirmiere> updateInfirmiere(
        @PathVariable(value = "id", required = false) final Long id,
        @Valid @RequestBody Infirmiere infirmiere
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/infirmieres/{id}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<Infirmiere> partialUpdateInfirmiere(
        @PathVariable(value = "id", required = false) final Long id,
        @NotNull @RequestBody Infirmiere infirmiere
    ) throws URISyntaxException {
        ...
        Optional<Infirmiere> result = infirmiereRepository
            .findById(infirmiere.getId())
            .map(existingInfirmiere -> {
                if (infirmiere.getIdInfirmiere() != null) {
                    existingInfirmiere.setIdInfirmiere(infirmiere.getIdInfirmiere());
                }
                if (infirmiere.getPrenom() != null) {
                    existingInfirmiere.setPrenom(infirmiere.getPrenom());
                }
                if (infirmiere.getNom() != null) {
                    existingInfirmiere.setNom(infirmiere.getNom());
                }

                return existingInfirmiere;
            })
            .map(infirmiereRepository::save);
     ...
     */
    @GetMapping("/infirmieres")
    public List<Infirmiere> getAllInfirmieres(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
