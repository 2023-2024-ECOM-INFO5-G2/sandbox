...
public class RappelRepositoryWithBagRelationshipsImpl implements RappelRepositoryWithBagRelationships {
    ...
    public Optional<Rappel> fetchBagRelationships(Optional<Rappel> rappel) {
        return rappel.map(this::fetchMedecins);
    }
    ...
    public List<Rappel> fetchBagRelationships(List<Rappel> rappels) {
        return Optional.of(rappels).map(this::fetchMedecins).orElse(Collections.emptyList());
    }

    Rappel fetchMedecins(Rappel result) {
        return entityManager
            .createQuery("select rappel from Rappel rappel left join fetch rappel.medecins where rappel.id = :id", Rappel.class)
            .setParameter("id", result.getId())
            .getSingleResult();
    }

    List<Rappel> fetchMedecins(List<Rappel> rappels) {
        ...
        List<Rappel> result = entityManager
            .createQuery("select rappel from Rappel rappel left join fetch rappel.medecins where rappel in :rappels", Rappel.class)
