...
/**
 * REST controller for managing {@link polytech.g02.ecom.domain.Patient}.
 */
@RestController
...
public class PatientResource {
    ...
    public List<Patient> getAllPatients(@RequestParam(required = false) String filter) {
        if ("rappel-is-null".equals(filter)) {
            log.debug("REST request to get all Patients where rappel is null");
            return StreamSupport
                .stream(patientRepository.findAll().spliterator(), false)
                .filter(patient -> patient.getRappel() == null)
                .toList();
        }
