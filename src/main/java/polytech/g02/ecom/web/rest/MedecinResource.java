...
/**
 * REST controller for managing {@link polytech.g02.ecom.domain.Medecin}.
 */
@RestController
...
public class MedecinResource {
    ...
    public ResponseEntity<Medecin> partialUpdateMedecin(
        ...
        Optional<Medecin> result = medecinRepository.findById(medecin.getId()).map(medecinRepository::save);
     ...
     */
    @GetMapping("")
    public List<Medecin> getAllMedecins(@RequestParam(required = false, defaultValue = "true") boolean eagerload) {
        log.debug("REST request to get all Medecins");
        if (eagerload) {
            return medecinRepository.findAllWithEagerRelationships();
        } else {
